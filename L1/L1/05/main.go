...
func imitation(N time.Duration) {
	t := time.Now()
	ctx, cancel := context.WithTimeout(context.Background(), N) //делаю таймер
	ch := make(chan string)
	var i = 1
	go func(i int) {
		for {
			ch <- fmt.Sprintf("data number %d was sent to the channel\n", i)
			i++ //бесконечные итерации отправки данынх в канал
	...
	for {
		...
		case data := <-ch:
			fmt.Print(data)
		case <-ctx.Done(): //после завершения таймера программа остановится
			cancel()
